...
public class CycleTimeReportService : ICycleTimeReportService
{
    ...
    public CycleTimeReportService(AppDbContext dbContext) => this.dbContext = dbContext;
    ...
    public async Task<ReportBatch<CycleTimeReportRow>> GetBatchAsync(
        CycleTimeReportFilter filter,
        int offset,
        int take,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (machineIds.Count > 0) query = query.Where(report => report.machine_id != null && machineIds.Contains(report.machine_id));
        ...
        if (normalizedSearch != null) query = query.Where(report => EF.Functions.ILike(report.lot_name!, $"%{normalizedSearch}%"));
    ...
    private async Task<ReportBatch<CycleTimeReportRow>> ExecuteBatchAsync(
        CycleTimeReportFilter filter,
        ...
        CancellationToken cancellationToken)
    {
        ...
        const string sql = """
            WITH grouped AS (
                SELECT
                    ...
                    COALESCE(SUM(pr.cycle_time_1) FILTER (WHERE pr.cycle_time_1 IS NOT NULL AND pr.cycle_time_1 <> 0), 0) AS "CycleTime1Total",
                    COUNT(*) FILTER (WHERE pr.cycle_time_1 IS NOT NULL AND pr.cycle_time_1 <> 0) AS "CycleTime1Count",
                    COALESCE(SUM(pr.cycle_time_2) FILTER (WHERE pr.cycle_time_2 IS NOT NULL AND pr.cycle_time_2 <> 0), 0) AS "CycleTime2Total",
                    COUNT(*) FILTER (WHERE pr.cycle_time_2 IS NOT NULL AND pr.cycle_time_2 <> 0) AS "CycleTime2Count",
                    COALESCE(SUM(pr.cycle_time_3) FILTER (WHERE pr.cycle_time_3 IS NOT NULL AND pr.cycle_time_3 <> 0), 0) AS "CycleTime3Total",
                    COUNT(*) FILTER (WHERE pr.cycle_time_3 IS NOT NULL AND pr.cycle_time_3 <> 0) AS "CycleTime3Count",
                ...
                GROUP BY mm.line, pr.lot_name, pr.mjs_id
                HAVING COUNT(*) FILTER (WHERE pr.cycle_time_1 IS NOT NULL AND pr.cycle_time_1 <> 0) > 0
                    OR COUNT(*) FILTER (WHERE pr.cycle_time_2 IS NOT NULL AND pr.cycle_time_2 <> 0) > 0
                    OR COUNT(*) FILTER (WHERE pr.cycle_time_3 IS NOT NULL AND pr.cycle_time_3 <> 0) > 0
            )
            SELECT "LineName", "ModelName", "GroupName", "CycleTime1Total", "CycleTime1Count",
                   "CycleTime2Total", "CycleTime2Count", "CycleTime3Total", "CycleTime3Count",
                   COUNT(*) OVER() AS "TotalRecords"
            FROM grouped
            ORDER BY "LatestReportDate" DESC, "LineName", "ModelName", "GroupName"
        ...
        var sqlRows = await dbContext.Database.SqlQueryRaw<CycleTimeReportSqlRow>(sql,
            ...
            .ToListAsync(cancellationToken);
        var rows = sqlRows.Select(row => new CycleTimeReportRow
        {
            LineName = row.LineName,
            ModelName = row.ModelName,
            GroupName = row.GroupName,
            CycleTime1 = Average(row.CycleTime1Total, row.CycleTime1Count),
            CycleTime2 = Average(row.CycleTime2Total, row.CycleTime2Count),
            CycleTime3 = Average(row.CycleTime3Total, row.CycleTime3Count)
        }).ToList();
    ...
    private static decimal? Average(decimal total, long count) => count > 0 ? Math.Round(total / count, 2) : null;
    ...
    private static CycleTimeReportViewModel CreateDatabaseErrorViewModel(CycleTimeReportFilter filter, Exception exception) => new()
    {
        Filter = filter,
        ModelOptions = ReportQueryParameters.SelectedOption(filter.ModelName),
